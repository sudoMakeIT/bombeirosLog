...
if($btnAdd)
{
    ...
    $query = "insert into honor (dia,tipo,elem,local,obs,note) values (?,?,?,?,?,?)";
    
    $exec = @mysqli_prepare($dbc,$query);
    
    @mysqli_stmt_bind_param($exec,"ssissi",$data,$tipo,$elem,$local,$obs,$ano);
    ...
    if($rows == 1)
    {
        ...
        header("Location: ../database.php#honor");
    }
    else
